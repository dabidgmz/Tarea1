    ...
    try {
        ...
        return response.status(500).json({ message: 'Error interno del servidor', error: error.message });
            ...
            try {
                ...
                return response.status(500).json({ message: 'Error al actualizar el usuario', error: error.message });
            }
            case 'DELETE':
            try {
            ...
            default:
            return response.status(400).json({ message: 'Mira bien soy el mencho y este método no admitido ala otra fijate bien si no amaneces de plomo' });
        }
    }
}
